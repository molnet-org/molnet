write_interaction_score_input <- function(combined_graphs, drug_target_edgelists, saving_path) {
  ...
  #'
  #' @description (INTERNAL) Writes the combined graphs and the drug target edge lists to files for passing them to the python interaction score script.
  #' Graphs are saved as `gml` file. Edgelists are saved as `tsv` file.
  #'
  #' @param combined_graphs A named list (elements `group1` and `group2`). Each element contains the entire combined network (layers + inter-layer connections) as iGraph graph object.
  #' @param drug_target_edgelists A named list (elements `group1` and `group2`). Each element contains the list of edges to be considered in the interaction score calculation as data frame (columns `from`, `to` and `weight`)
...
calculate_interaction_score <- function(max_path_length, total_edges, loading_path, python_executable = "python3", script_path = NULL, int_score_mode="auto") {
  ...
  #'
  #' @param max_path_length The maximum length of simple paths to consider when computing the interaction score
  #' @param total_edges vector with total edges in each group
  #' @param loading_path Directory to use for writing intermediate data when passing input and output between Python and R
  #' @param python_executable Python command or path to Python executable to use
  #' @param script_path Path to the interaction score Python script. Set NULL to use package internal script (default).
...
load_interaction_score_output <- function(loading_path) {
  ...
  #'
  #' (INTERNAL) Loads data generated by \code{\link{calculate_interaction_score}}. Output files are graphs in gml format for both groups.
  #'
  #' @param loading_path Directory to load from
  #'
  #' @return A named list (elements `group1` and `group2`). Each element contains an iGraph object containing the interaction score as edge attribute.
  #' @export
  #'
  groups <- c("group1", "group2")
  graphs <- list()
