make_layer <- function(name, data_group1, data_group2, identifier_group1, identifier_group2) {
  ...
  #'
  #' Helper function to transform input data to required pipeline input format. Additionally, the supplied input is checked. Allows easy conversion of raw data into the structure accepted by \code{\link{start_pipeline}}.
  #'
  #' @param name Character string. Names the layer.
  #' @param data_group1,data_group2 Data frame containing raw molecular data of each group (each stratum). Analyzed components (e.g., genes) in columns, samples (e.g. patients) in rows.
  #' @param identifier_group1,identifier_group2 Data frame containing component identifiers (columns) of each component (rows) in the same order as the molecular data frame of each group. These identifiers are used to (a) interconnect graphs and (b) match drugs to drug targets. Must contain a column `type` which identifies the nature of the component (e.g., "protein")
  #'
  #' @return Named list containing the supplied data for each group (i.e., the dataset for one layer), that can be supplied to \code{\link{start_pipeline}} and `name` giving the name of the layer. Each sublist contains the `data` and the `identifiers`.
  #' @export
  #'
  #' @examples
  #' data(mrna_data)
...
make_connection <- function(from, to, connect_on, weight = 1, group = "both") {
  ...
  #' The connection can be based on IDs present in the identifiers of both layer or an interaction table containing mapping the connections and edge weights.
  #' Additionally, the supplied input is checked. Allows easy conversion of raw data into the structure accepted by \code{\link{start_pipeline}}.
  #'
  #' __IMPORTANT:__ if a connection is established based on \code{id} this ID has to be present in the identifiers of both layers, have to be named identically and IDs have to be formatted identically as these are matched by an inner join operation (refer to \code{\link{make_layer}}).
  #'
  #' @param from Character string referring to the name of the layer **from** which the connection should be established
  #' @param to Character string referring to the name of the layer **to** which the connection should be established
  #' @param connect_on Specifies how the two layers should be connected. This can be based on a mutual ID or a table specifying interactions:
  #' * __Mutual ID__: Character string specifying the name of an identifier that is present in both layers (e.g., `NCBI ID` to connect proteins and mRNA).
  #' * __Interaction table__: A table mapping two identifiers of two layers. The columns have exactly the same names as the identifiers of the layers. Has to contain an additional column specifying the weight between two components/nodes (see `weight` argument)
  #' @param weight Specifies the edge weight between the layers. This can be supplied as a number applied to every connection or a column of the interaction table:
  #' * __Fixed weight__: number specifying the weight of every connection between the layers.
  #' * __Based on interaction table__: Character string specifying the name of a column in the table passed as the `by` parameter which is used as edge weight.
  #' @param group Group for which to apply the connection. One of `both`, `1` or `2`.
  #' @return A named list (i.e., an inter-layer connection), that can be supplied to \code{\link{start_pipeline}}.
  #' @export
  #'
  #' @examples
...
make_drug_target <- function(target_molecules, interaction_table, match_on) {
  ...
  #'
  #' @description Function to transform input data to required input format for \code{\link{start_pipeline}}. Here the data needed to define drug-target interactions is formatted. When the reformatted output is passed to \code{\link{start_pipeline}} as \code{drug_target_interaction} argument, the differential drug response score will be calculated for all the supplied drugs in \code{interaction_table}.
  #'
  #' @param target_molecules Name of layer containing the drug targets. This name has to match the corresponding named item in the list of layers supplied to \code{\link{start_pipeline}}.
  #' @param interaction_table Data frame. Has to contain two columns. Additional columns will be ignored in the pipeline.
  #' * A column called `drug_name` containing names or identifiers of drugs
  #' * A column with a name that matches an identifier in the layer supplied in `target_molecules`. For example, if drugs target proteins and an identifier called `ncbi_id` was supplied in layer building of the protein layer (\code{\link{make_layer}}), this column should be called `ncbi_id` and contain the corresponding IDs of protein-drug targets. Any other ID present in the constructed layer can be used.
  #' @param match_on Column name of the data frame supplied in `interaction_table` that is used for matching drugs and target nodes in the graph (e.g. `ncbi_id`).
  #'
...
start_pipeline <- function(layers, inter_layer_connections, drug_target_interaction, settings) {
  ...
  #'
  #' This wrapper function executes all necessary steps to generate differential drug response scores from the formatted input data. The following input data is required (and detailed below):
  ...
  #'
  #' As this function runs through all steps of the molnet-pipeline it can take a long to complete, especially if the supplied molecular data is in large dimensions. Several prompts will be printed to supply information on how the pipeline is proceeding. Calculation of the interaction score by \code{\link{interaction_score}} requires saving large-scale graphs to file and calls a python script. This handover may take time.
  #'
  ...
  #'
  #' @param layers Named list with different network layers containing data and identifiers for both groups. The required input format is a list with names corresponding to the content of the respective layer (e.g., "protein"). Each named element has to contain the molecular data and corresponding identifiers formatted by  \code{\link{make_layer}}.
  #' @param inter_layer_connections A list with specified inter-layer connections. This list contains one or more elements defining individual inter-layer connections created by \code{\link{make_connection}}.
  #' @param drug_target_interaction A list specifying drug-target interactions for drug response score computation. The required input format of this list is created by \code{\link{make_drug_target}}. The drug response score is calculated for all drugs contained in this object.
  #' @param settings A named list containing pipeline settings. The settings list has to be initialized by \code{\link{molnet_settings}}. Items in the named list can be adjusted as desired.
  #' @return Data frame containing drug name and associated differential drug response score. If no target is found for a specific drug, \code{NA} is returned as a score.
  #' @export
  #' @examples
  #' data(drug_gene_interactions)
  #' data(layers_example)
  ...
  #' settings <- molnet_settings(handling_missing_data = list(default = "pairwise.complete.obs",mrna = "all.obs"),save_individual_graphs = FALSE,
...
generate_individual_graphs <- function(layers, settings) {
  ...
  #' `annotations`.
  #'
  #' @param layers Named list with different network layers containing data and identifiers for both groups (generated from \code{\link{make_layer}})
  #' @param settings A named list containing pipeline settings
  #'
  #' @return A nested named list with first-level elements `graphs` and `annotations`. The second level elements are `group1` and `group2`. These contain a list of iGraph objects (`graphs`) and data frames (`annotations`) mapping the graph node IDs to biological identifiers.
  #' @export
  #'
  #' @examples
  #' data(layers_example)
...
generate_combined_graphs <- function(graphs, annotations, inter_layer_connections, settings) {
  ...
  #'
  #' @param graphs A named list (elements `group1` and `group2`). Each element contains a list of iGraph objects (output of \code{\link{generate_individual_graphs}}).
  #' @param annotations A named list (elements `group1` and `group2`). Each element contains a list of data frames mapping each node IDs to identifiers (output of \code{\link{generate_individual_graphs}}).
  #' @param inter_layer_connections Named list with specified inter-layer connections. Names are layer names and elements are connections (\link{make_connection}).
  #' @param settings A named list containing pipeline settings
  #'
  #' @return A named list (elements `graphs` and `annotations` and sub-elements `$group1` and `$group2`). Contains the igraph objects of the combined network and their annotations for both groups.
  #' @export
  #'
  #' @examples
  #' data(individual_graphs_example)
  #' individual_graphs <- individual_graphs_example
  #' inter_layer_connections <- list(molnet::make_connection(from="mrna", to="protein", connect_on="gene_name", weight=1))
  ...
  for(n_connection in c(1:length(inter_layer_connections))) {
    ...
    # connections can be established by shared IDs ('id') between networks or by a given interaction table ('table')
    if(connection$by == 'id') {
      cat("by id.\n")
      # iterate over groups
      for(group in groups) {
...
determine_drug_targets <- function(graphs, annotations, drug_target_interaction, settings) {
  ...
  #'
  #' @description Finds node IDs of network nodes in `graphs` that are targeted by a drug in `drug_target_interaction`. Returns list of node ids and list of adjacent edges.
  #'
  #' @param graphs A named list with elements `group1` and `group2` containing the combined graphs of each group as iGraph object.
  #' @param annotations List of data frames that map node IDs to identifiers. Contains `all` (unique identifiers across the whole data) and `group1` and `group2` containing identifiers specific to the strata.
  #' @param drug_target_interaction Named list specifying drug target interactions for drug response score computation
  #' @param settings A named list containing pipeline settings
  #'
  #' @return A named list with elements `drug_targets` and `drug_target_edge_list`.
  #' * `targets` is a named list with elements `target_nodes` and `drugs_to_target_nodes`. `target_nodes` is a data frame with column `node_id` (unique node IDs in the iGraph object targeted by drugs) and columns `group1` and `group2` (boolean values specifying whether the node is contained in the combined graph of the group). Element `drugs_to_target_nodes` contains a named list mapping drug names to a vector of their target node IDs.
  #' * `drug_target_edge_list` contains elements `group1` and `group2` containing each a list of edges adjacent to drug target nodes.
  #' @export
  #'
  #' @examples
  #' data(drug_gene_interactions)
...
interaction_score <- function(graphs, drug_target_edgelists, settings) {
  ...
  #' DISCLAIMER: Depending on the operating system Python comes pre-installed or has to be installed manually.
  #' Please pay attention to the version and the executable used (python/python3 or homebrew python). You
  #' can use the `python_executable` setting to specify the command or path.
  #'
  #' @param graphs A named list (elements `group1` and `group2`). Each element contains the combined graph for its group.
  #' @param drug_target_edgelists A named list (elements `group1` and `group2`). Each element contains the list of edges adjacent to drug targets as a data frame (columns `from`, `to` and `weight`)
  #' @param settings A named list containing pipeline settings
  #'
  #' @return A named list (elements `group1` and `group2`). Each element contains an iGraph object containing the interaction score as weight.
  #' @export
  #'
  #' @examples
  #' data(combined_graphs_example)
...
get_drug_response_score <- function(differential_graph, drug_targets, interaction_table) {
  ...
  #'
  #' @description This function takes the differential graph (generated in \code{\link{differential_score}}), the a drug targets object (containing target node names and drugs and their targets; generated in \code{\link{determine_drug_targets}}) and the supplied drug-target interaction table (formatted in \code{\link{make_drug_target}}) to calculate the differential drug response score. The score is the median of all differential scores of the edges adjacent to all drug target nodes of a particular drug.
  #'
  #' @param differential_graph iGraph graph object containing differential scores for all edges. Output of \code{\link{differential_score}}
  #' @param drug_targets Named list containing two elements (`target_nodes` and `drugs_to_target_nodes`). Output of \code{\link{determine_drug_targets}}. `target_nodes` is a vector containing network node names of the nodes that are targeted by the available drugs. `drugs_to_target_nodes` is a dictionary-like list that maps drugs to the nodes that they target.
  #' @param interaction_table Data frame. Element `interaction_table` of `drug_target_interaction` created by \code{\link{make_drug_target}}. Contains at least two columns: `drug_name` containing names of drugs and a column named with an identifier present in the targeted layer.
  #'
  #' @export
  #'
