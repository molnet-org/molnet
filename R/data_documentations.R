...
#' @source Krug, Karsten et al. “Proteogenomic Landscape of Breast Cancer Tumorigenesis and Targeted Therapy.” Cell vol. 183,5 (2020): 1436-1456.e31. doi:10.1016/j.cell.2020.10.036
"layers_example"

#' Individual graphs
#'
#' Exemplary intermediate pipeline output: Individual graphs example data built by \code{\link{generate_individual_graphs}}. Graphs were created by correlation computation and reduced by the 'p_value' reduction method (default settings of \code{\link{molnet_settings}}. A subset of the original data by Krug et al., 2020 and randomly sampled metabolite data (\code{\link{layers_example}}) was used to generate graphs. They were created from data stratified by estrogen receptor (ER) status: group1 contains data of ER+ patients and group2 of ER- patients.
#'
#'
...
#'     }
#'   \item{annotations}{A named list containing data frames of mappings of assigned node IDs to the user-provided component identifiers for nodes in group1 or group2 and all nodes}
#'   \describe{
#'   \item{group1}{}
#'   \item{group2}{}
#'   \item{all}{}
...
#' Combined graphs
#'
#' Exemplary intermediate pipeline output: Combined graphs example data built by \code{\link{generate_combined_graphs}}. Graphs were created with default settings of \code{\link{molnet_settings}}.  A subset of the original data by Krug et al., 2020 and randomly sampled metabolite data was used to generate graphs \code{\link{layers_example}}. They were created from data stratified by estrogen receptor (ER) status: group1 contains data of ER+ patients and group2 of ER- patients.
#'
#'
#' @format A named list with 2 items.
#' \describe{
...
#'
#' Exemplary intermediate pipeline output: Drug targets detected in the combined graphs. A named list with elements `drug_targets` and `edgelist`. This was created from \code{\link{determine_drug_targets}} using the default settings given by \code{\link{molnet_settings}}. Drug-gene interactions to calculate this output were used from The Drug Gene Interaction Database.
#'
#' @format A named list with 2 items.
#' \describe{
#'   \item{targets}{A named list}
#'   \describe{
#'   \item{target_nodes}{data frame with column `node_id` (unique node IDs in the graph targeted by drugs) and columns `group1` and `group2` (boolean values specifying whether the node is contained in the combined graph of the group)}
#'   \item{drugs_to_target_nodes}{Element `drugs_to_target_nodes` contains a named list mapping drug names to a vector of their target node IDs.}
#'   }
#'   \item{edgelist}{Contains elements `group1` and `group2` containing each a data frame of edges adjacent to drug target nodes each. Each edgelist data frame contains columns `from`, `to` and `weight`.}
#' }
...
#'
#' Exemplary intermediate pipeline output: Interaction score graphs example data built by \code{\link{interaction_score}}. A named list (elements `group1` and `group2`). Each element contains an iGraph object containing the interaction score as weight. This was computed using a subset of the data published by Krug et al., 2020 and randomly sampled metabolite data \code{\link{layers_example}}.
#'
#' @format A named list with 2 items.
#' \describe{
...
#' Drug response score
#'
#' Exemplary final pipeline output: Drug response score data frame. This contains drugs and the calculated differential drug response score. The score was calculated by \code{\link{get_drug_response_score}}.
#'
#'  The original data used to compute this object was data published by Krug et al., 2020 and randomly sampled metabolite data \code{\link{layers_example}}.. Drug-gene interactions to calculate this output were used from The Drug Gene Interaction Database.
#'
...
#'
#' @description Data frame providing interactions of metabolites and proteins. The data was taken from the STITCH Database.
#'
...
#'     \item{identifiers}{one column per identifier, rows in the same order as the protein order in 'data', identifiers: RefSeq ID, gene name}
#'     }
#'   \item{group2}{ER- data}
#'   \describe{
#'     \item{data}{see above}
...
#'     \item{identifiers}{one column per identifier, rows in the same order as the mRNA order in 'data', identifiers: gene name}
#'     }
#'   \item{group2}{ER- data}
#'   \describe{
#'     \item{data}{see above}
...
#'     \item{identifiers}{one column per identifier, rows in the same order as the phoshosite order in 'data', identifiers: Phosphosite ID, RefSeq ID, gene name}
#'     }
#'   \item{group2}{ER- data}
#'   \describe{
#'     \item{data}{see above}
...
#'     \item{identifiers}{one column per identifier, rows in the same order as the metabolite order in 'data', identifiers: biochemical name, METABOLON ID, Pubchem ID}
#'     }
#'   \item{group2}{ER- data}
#'   \describe{
#'     \item{data}{see above}
